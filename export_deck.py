...
def _title_slide(prs, data: DeckData, brand_name: str, colours):
    ...
    try:
        ...
        if logo is not None:
            ...
            slide.shapes.add_picture(str(logo), Inches(0.8), Inches(0.5),
                                     height=Inches(1.15))
            _logo_placed = True
    except Exception:
    ...
    _add_text(slide, data.headline, 0.8, 1.9, 11.5, 1.2, size=40, bold=True)
    ...
    if subtitle_bits:
        _add_text(slide, "  ·  ".join(subtitle_bits), 0.8, 3.1, 11.5, 0.5, size=18)

    as_of = data.as_of or datetime.date.today()
    _add_text(slide, f"Prepared {as_of:%d %B %Y} · Generated by {brand_name}",
              0.8, 3.7, 8, 0.4, size=14, colour=colours.text_muted)
